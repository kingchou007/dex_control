...
# ROS 2 Node
# -----------------------------------------------------------------------------
class Ros2AvStreamer(Node):
    def __init__(self, image_topic: str, frame_id: str, qos_depth: int):
        super().__init__('gopro_ros2_streamer')
        ...
        self.image_pub = self.create_publisher(CompressedImage, f"{image_topic}/compressed", qos)
        
        self.get_logger().info(f"Publishing COMPRESSED images to {image_topic}/compressed")

    def publish_image(self, frame: np.ndarray, stamp: Optional[float] = None):
        if frame is None:
            return
...
@click.option('--audio_device_id', default='0', type=str, help="Unused (legacy)")
@click.option('--display', default=None, help="X11 Display")
@click.option('--image_topic', default='/gopro/image_raw', help="Base ROS topic for images")
@click.option('--audio_topic', default='/gopro/audio_raw', help="Unused (legacy)")
@click.option('--frame_id', default='gopro_camera', help="TF Frame ID")
@click.option('--ros_qos_depth', default=10, type=int, help="QoS Depth")
@click.option('--audio_block_ms', default=20, type=int, help="Unused (legacy)")
@click.option('--no_gui', is_flag=True, help="Disable local video preview")
def main(camera_idx, dev_path, audio_device_id, display, image_topic, 
         audio_topic, frame_id, ros_qos_depth, audio_block_ms, no_gui):
    ...
    # 2. Setup ROS
    rclpy.init()
    # Note: audio_topic is intentionally ignored as audio logic was disabled
    node = Ros2AvStreamer(image_topic, frame_id, ros_qos_depth)
    
    print("Streaming (1080p60 Compressed)... Press 'q' to stop.")
    ...
    try:
        while rclpy.ok():
            ...
            if ret and frame is not None:
                ...
                # Slight delay if no frame to prevent tight loop
                time.sleep(0.001)
